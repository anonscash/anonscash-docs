...
• enable clean wallet hygiene within an application’s workflow  

## Summary

anons.cash integrates through standard Solana transactions and a simple proof-
submission flow. No custody, no accounts, and no off-chain state are required.
External tools can embed private deposits and withdrawals while keeping their
own systems minimal and safe.
